<?php 
    ...
    function cek_hari($tanggal) {
		...
		$hari = get_hari($day);
        $ci = & get_instance();
		$jadwal = $ci->db->query("SELECT * FROM jam WHERE hari = '$hari' AND buka <= '$waktu' AND tutup >= '$waktu'")->num_rows();
        return $jadwal;
    }

    function add_minute($tgl) {
        $date = date_create($tgl);
        date_add($date, date_interval_create_from_date_string('45 minute'));
        $tgl_temp = date_format($date, 'Y-m-d H:i:s');
        return $tgl_temp;
    }

    function penentuan_jadwal($tgl) {
        // date_default_timezone_set("Asia/Jayapura");
        // $tgl = date('Y-m-d H:i:s');
        $day = date('D', strtotime($tgl));
		$hari = get_hari($day);
        $ci = & get_instance();
        // $tgl_treatment = add_minute($tgl);
        // $cek_hari = cek_hari($tgl_treatment);

        // if ($cek_hari > 0) {
        //     $tgl_treatment = $tgl_treatment;
        // } else {
        //     do {
        //         $tgl_treatment = add_minute($tgl_treatment);
        //         $i = cek_hari($tgl_treatment);
        //     } while ($i < 1);
        // }
        ...
        $tahun = substr($tgl,0,4);
        $q = $ci->db->query("SELECT MAX(RIGHT(no_antrian,3)) as kd_max FROM pemesanan WHERE YEAR(tanggal_treatment) = '$tahun' AND MONTH(tanggal_treatment) = '$bulan' AND DAY(tanggal_treatment) = '$tanggal'");
        $k = $q->row_array();
        if($q->num_rows() > 0){
            $tmp = ((int)$k['kd_max'])+1;
            $kd = sprintf("%03s", $tmp);
    ...
    function selisih_waktu($tgl) {
        ...
        $days    =(int)((mktime ($jam,$menit,0,$bulan,$tanggal,$tahun) - time())/60);
        return $days;
    }
